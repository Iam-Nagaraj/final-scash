...
class MerchantController extends Controller
{
    ...
    public function detail(Request $request)
    {
        try {
            ...
            $RepeatCashbackModel = RepeatCashback::where('user_id', Auth::user()->id)->where('merchant_id', $request->id)->first();
            if(!empty($RepeatCashbackModel) && !empty($cashbackRule) && !empty($cashbackRule->rp_total_amount) ){
                if($RepeatCashbackModel->rp_cashback_balance >= $cashbackRule->rp_total_amount){
                    $progressPercentage = '100';
                } else {
                    $progressPercentage = ($RepeatCashbackModel->rp_cashback_balance * 100 ) / $cashbackRule->rp_total_amount ;
                }
