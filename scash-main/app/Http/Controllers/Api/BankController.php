    ...
    **/
    ...
    public function makeFundTransfer(FundTransferRequest $request)
	{
        ...
        try {
            ...
            if($toUserData->role_id ==  getConfigConstant('MERCHANT_ROLE_ID') || $toUserData->role_id ==  getConfigConstant('STORE_ROLE_ID')){
                ...
                if($toUserData->role_id ==  getConfigConstant('MERCHANT_ROLE_ID') || $toUserData->role_id ==  getConfigConstant('STORE_ROLE_ID')){
                    $cashback_percentage = $cashback_business_type->percentage;
                } else {
                ...
                if(isset($cashbackRule)){
                    ...
                    $cashback_percentage = $cashbackRule->standard_cashback_percentage + $cashback_percentage;  
                    // added admin standard cashback
                    $standard_cashback = round(($amount * $cashback_percentage)/100,2);

                    if( ($amount >= $cashbackRule->ts_total_amount) && ($cashbackRule->ts_status == CashbackRule::RULEACTIVE)){ 
                        // for increase transactional amount
                    ...
                    $RepeatCashbackModel = RepeatCashback::where('user_id', Auth::user()->id)->where('merchant_id', $request->to_user_id)->first();
                    if(empty($RepeatCashbackModel)){
                        $newRepeatCashback = new RepeatCashback();
                        $newRepeatCashback->merchant_id = $request->to_user_id;
                        $newRepeatCashback->user_id = Auth::user()->id;
                        $newRepeatCashback->rp_cashback_balance = 0;
                        $newRepeatCashback->save();
                    }
                    $RepeatCashbackModel = RepeatCashback::where('user_id', Auth::user()->id)->where('merchant_id', $request->to_user_id)->first();
                    
                    $RepeatCashbackModel->rp_cashback_balance = $RepeatCashbackModel->rp_cashback_balance + $amount;
                    if( ($RepeatCashbackModel->rp_cashback_balance >= $cashbackRule->rp_total_amount) && ($cashbackRule->rp_status == CashbackRule::RULEACTIVE)){ 
                        // for repeat transactional amount
                        $cashback_percentage = $cashback_percentage +  $cashbackRule->rp_extra_percentage;
                        // $RepeatCashbackModel->rp_cashback_balance = $RepeatCashbackModel->rp_cashback_balance - $cashbackRule->rp_total_amount;
                        $RepeatCashbackModel->rp_cashback_balance = 0;
                        $rp_extra_cashback = round(($amount * $cashbackRule->rp_extra_percentage)/100,2);
                    }
                    $RepeatCashbackModel->save();
                }
            ...
            if($toUserData->role_id == User::ROLE_STORE)
            {
                ...
                if($MerchantStore && $MerchantStore->merchant_id){
                    ...
                    if($storeMerchantWallet && $storeMerchantWallet->user_id){
                        $storeMerchantWallet->balance = $storeMerchantWallet->balance + $request->amount + $request_cashback; // update receiver user wallet
