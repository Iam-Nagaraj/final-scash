...
class User extends Authenticatable
{
    ...
    {
		if($this->role_id == 5){
			$storeData = MerchantStore::where('user_id', $this->id)->first();
			if($storeData){
				$UserMedia = UserMedia::where('user_id', $storeData->merchant_id)->where('type', UserMedia::TYPE_IMAGE)->first();
			}
	...
	{
		return $this->hasOne(User::class, 'id')->where('role_id', getConfigConstant('STORE_ROLE_ID'));
	}
	...
	{
		$data = self::with(['media', 'address', 'kyc']);
		if ($id) {
			$data->where('id', $id);
		}
